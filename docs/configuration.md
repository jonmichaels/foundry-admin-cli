...
`mode = "remote"` or `mode = "http-only"` means local-only commands fail fast with an actionable error. A remote URL alone is not enough for package/world filesystem mutations or PM2 lifecycle commands, and v0.4 does not include an SSH orchestration wrapper.
